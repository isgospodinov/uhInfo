...
public:
	...
	void SetDefSize() {if(plMw)set_default_size((2 * plMw->get_width() / 3), (plMw->get_height() - (2 * plMw->get_height() / 3)));}
	bool Wnd_close_handler();
	virtual void on_show() override;
	void stop_timer() {if(l_timer) {l_timer.get()->disconnect();l_timer = std::unique_ptr<sigc::connection>(nullptr);}}
