...
using Sensor_node = struct _Sensor_node : public Sensor_statistic{
      _Sensor_node(int fn,std::string l,sensors_feature_type sft,bool sv) : Sensor_statistic(sv),feature_number(fn),label(l),sntype(sft) {}
      int feature_number;
      std::string label;
...
public :
    ...
    virtual CDrawArea::DRAWVECTORPLUS SensorStatisticToggle(bool status,Glib::ustring color,Glib::ustring node,Glib::ustring sensor,Glib::ustring nodeid,int sensorid,double **max = nullptr) override;
private:
